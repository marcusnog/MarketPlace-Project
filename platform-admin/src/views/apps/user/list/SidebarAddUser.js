...
const SidebarAddUser = ({ currentItem, open, toggleSidebar }) => {
  ...
  const dataSchema = yup.object().shape({
    ...
    newclientuser: yup.boolean().required().default()
  })
  ...
  const setCurrentState = (currentItem) => {
    if (!!currentItem) {
      setData({
        ...
        cliente: currentItem.cliente,
        newclientuser: currentItem.newclientuser
      })
      setFirstRun(true)
    } else {
  ...
  const onSubmit = values => {
    if (isObjEmpty(errors)) {
      ...
      const { id, nickname, email, password, phone, idProfile, idSystem, newclientuser  } = values
      dispatch(!id ? addUser({ nickname, email, phone, password, idProfile: idProfile?.value, idSystem: idSystem?.value, clientId: loggedUserData.ClientId, newclientuser }) : updateUser({ id, nickname, phone, idProfile: idProfile?.value }))
      handleToggleSidebar()
      setCurrentState(undefined)
    }
  }
          ...
          <FormGroup>
            ...
            <Controller
              ...
              as={<input placeholder="*********"
                className={classnames('form-control', { 'is-invalid': !!isSubmitted && !!errors['password'] })} />}
            />
            {/* {errors && errors.phone && <FormFeedback>{errors.phone.message}</FormFeedback>} */}
          </FormGroup>
          <FormGroup className={classnames({ hidden: !!data.id })}>
