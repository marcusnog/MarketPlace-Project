...
// ** Store & Actions
import { deleteClient, editClient } from '../store/action'
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex flex-grow-1 justify-content-left align-items-center'>
        ...
        <div className='d-flex flex-column'>
          <span className='font-weight-bold'>{(row.documents || []).find((d) => d.type === "CNPJ")?.value}</span>
        </div>
      </div>
    )
  },
  ...
  },
  {
    ...
    cell: row => (
      <UncontrolledDropdown>
        ...
        <DropdownMenu right>
          <DropdownItem className='w-100' onClick={() => store.dispatch(deleteClient(row.id))}>
          ...
          <DropdownItem className='w-100' onClick={() => store.dispatch(editClient(row.id))}>
